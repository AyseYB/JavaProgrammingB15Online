...
public class HomeWorkFastFoodRestaurants {
    ...
    public static void main(String[] args) {
        ...
        for(String each: restaurants){
            if(each.contains("McDonald's"))
            ++mcdonalsCount;
        }
        System.out.println("there is " + mcdonalsCount + " MCDonalds in the list");
        ...
        int popeyesCount = 0;
        for( String each :restaurants ){
            if (each.contains("Popeye's")){
                ++popeyesCount;
            }
        }
       System.out.println("there is " + popeyesCount + " popeyes in the list.");

        int dominosCount = 0;
        for( String eachDominos: restaurants){
            if(eachDominos.contains("Domino's")){
                ++dominosCount;

            }
        }
        System.out.println("there are " + dominosCount + " Domino's in the list.");
        int tacoBellCount = 0;
        for ( String each : restaurants){
            if(each.contains("Taco Bell")){
                ++tacoBellCount;
            }
        }
        System.out.println("there are " + tacoBellCount + " Taco Bell in the list.");
        ...
        int num= 0;
        for(String each : restaurants){
            if(each.contains("McDonald's") && each.contains("TX")){
                ++num;
...
//        for (int i = 0; i <restaurants.length ; i++) {
//            String[] each = restaurants[i].split(",");
//            System.out.println(each[2] + " ->" + each[1] + ", " + each[4]);
        //}

        for (int i = 0 ; i< restaurants.length ; i++){
            String[] allRestaurants =restaurants[i].split(",");
            System.out.println(allRestaurants[2] + " ->" + allRestaurants[1] + ", " + allRestaurants[4]);
        }
    }
}
